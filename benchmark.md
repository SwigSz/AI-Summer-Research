...
I did not run the VRAM-wall case here. A model larger than the 4060's 8 GB (for example
`qwen2.5:14b` at ~9 GB Q4) would spill to shared memory and drop decode into single digits,
which would show capacity as a separate bottleneck from bandwidth. That run is covered in
[Game Day](gameday.md).
